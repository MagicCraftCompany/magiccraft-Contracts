...
contract MCRTStake is Ownable {
    ...
    struct USER {
        ...
        uint256 claimAbleTokensForStaking;
        uint256 claimAbleTokensForPoint;
    ...
    function STAKE(
        ...
        if (stakers[user][lockingPeriod].initialized) {
            ...
            if (option == 0) {
                ...
                if (lockingPeriod == 180) {
                    User.claimAbleRewardItem = User.claimAbleRewardItem.add(
                        tokens.div(tokenAmountForItemPoint)
                    );
                } else if (lockingPeriod == uint256(365)) {
                    User.claimAbleRewardItem = User.claimAbleRewardItem.add(
                        tokens.div(tokenAmountForCharacterPoint)
                    );
                } else if (lockingPeriod == 1095) {
                    User.claimAbleRewardItem = User.claimAbleRewardItem.add(
                        tokens.div(tokenAmountForLandPoint)
                    );
                } else if (lockingPeriod == 1825) {
                    User.claimAbleRewardItem = User.claimAbleRewardItem.add(
                        tokens.div(tokenAmountForLandPoint).mul(2)
                    );
                }
            }
            ...
            if (option == 0) {
                ...
                newStaking.claimAbleTokensForPoint = 0;
                newStaking.claimAbleTokensForStaking = 0;
                ...
                newPoint.claimAbleTokensForPoint = 0;
                newPoint.claimAbleTokensForStaking = 0;
                newPoint.apr = rewardTokenPercentage[lockingPeriod];
                if (lockingPeriod == 180) {
                    newPoint.claimAbleRewardItem = tokens.div(tokenAmountForItemPoint);
                } else if (lockingPeriod == uint256(365)) {
                    newPoint.claimAbleRewardItem = tokens.div(tokenAmountForCharacterPoint);
                } else if (lockingPeriod == 1095) {
                    newPoint.claimAbleRewardItem = tokens.div(tokenAmountForLandPoint);
                } else if (lockingPeriod == 1825) {
                    newPoint.claimAbleRewardItem = tokens.div(tokenAmountForLandPoint).mul(2);
                }
    ...
    function _poolUpdate(uint256 lockingPeriod) private {
        if (stakers[_msgSender()][lockingPeriod].initialized == true) {
            ...
            if (
                ...
                updateStaking.claimAbleTokensForStaking = updateStaking.stakedTokens;
                updateStaking.claimAbleTokensForPoint = updateStaking.tokenForPoints;
    ...
    function ClaimRewardPerPeriod(uint8 option, uint256 period) external {
        ...
        if (option == 0) {
            ...
            if (period == 180) {
                ...
                require(
                    totalRewardCharacterPoint > 0,
                    "Error: There isn't Character Point to be claimed"
                );
            } else if (period == 1095) {
    ...
    function WithdrawForStakingPerPeriod(uint256 lockingPeriod, uint8 option) external {
        ...
        _poolUpdate(lockingPeriod);
        if (option == 0) {
            uint256 withdrawTokens = stakers[_msgSender()][lockingPeriod].claimAbleTokensForStaking;
            require(
                withdrawTokens > 0,
            ...
            stakers[_msgSender()][lockingPeriod].stakedTokens = stakers[_msgSender()][lockingPeriod]
                .stakedTokens
                .sub(stakers[_msgSender()][lockingPeriod].claimAbleTokensForStaking);
            totalTokensForStaking = totalTokensForStaking.sub(
                stakers[_msgSender()][lockingPeriod].claimAbleTokensForStaking
            );

            stakers[_msgSender()][lockingPeriod].claimAbleTokensForStaking = 0;
            ...
            uint256 withdrawTokens = stakers[_msgSender()][lockingPeriod].claimAbleTokensForPoint;
            require(
                withdrawTokens > 0,
                "Error: there aren't withdrawable tokens or they aren't limited to staking time"
            );
            ...
            stakers[_msgSender()][lockingPeriod].tokenForPoints = stakers[_msgSender()][
                lockingPeriod
            ].tokenForPoints.sub(stakers[_msgSender()][lockingPeriod].claimAbleTokensForPoint);
            totalTokensForPoint = totalTokensForPoint.sub(
                stakers[_msgSender()][lockingPeriod].claimAbleTokensForPoint
            );
            stakers[_msgSender()][lockingPeriod].claimAbleTokensForPoint = 0;
            emit UNSTAKED(_msgSender(), lockingPeriod, withdrawTokens);
        }
    }
        ...
        )
    {
        ...
        return (apy, stakedMCRTOption, currentTime, currentreward, startTime);
    }
    ...
    function emergencySaveLostTokensForUser() external {
        ...
        for (uint256 i = 0; i < 6; i++) {
            if (stakers[_msgSender()][lockingPeriod[i]].initialized) {
                ...
                stakers[_msgSender()][lockingPeriod[i]].claimAbleTokensForStaking = 0;
                totalTokenForStaker = totalTokenForStaker.add(
                ...
                stakers[_msgSender()][lockingPeriod[i]].claimAbleTokensForPoint = 0;
